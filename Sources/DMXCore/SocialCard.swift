...
public enum SocialCard {
    ...
    public static let width: CGFloat = 1280
    public static let height: CGFloat = 640

    private static let title = "Nimbus DMX Helper"
    private static let pitch = "A toy DMX controller for macOS — a native SwiftUI app plus "
        + "a CLI, driving lights through an Enttec DMX USB Pro."
    private static let slug = "github.com/njoubert/nimbus-dmx-helper"
    private static let accent = NSColor(srgbRed: 0.941, green: 0.635, blue: 0.290, alpha: 1)
    ...
    private static let panelWidth: CGFloat = 800
    /// How far past that fill scale to push it, and what survives the crop:
    /// (0,0) is the screenshot's bottom-left, (1,1) its top-right.
    private static let shotZoom: CGFloat = 1.45      // a wide window is mostly empty grey at 1 — crop into the sliders
    private static let shotAnchor = CGPoint(x: 0.22, y: 0.88)     // ends on the pane divider: cutting through the debug column reads as damage
    ...
    public static func write(to path: String, icon iconPath: String, screenshot shotPath: String) throws {
